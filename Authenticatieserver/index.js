...
var oauthserver = require('../../'); // Would be: 'oauth2-server'
var fs = require('fs');
var https = require('https');
var app = express();

var privateKey  = fs.readFileSync('auth_server.key');
var certificate = fs.readFileSync('auth_server.crt');
var sub_ca = fs.readFileSync('sub-ca.crt');
var root_ca = fs.readFileSync('root-ca.crt');
var passPhrase = 'abc123';
var options = {ca: [sub_ca, root_ca], key: privateKey, cert: certificate, passphrase: passPhrase};
...
app.all('/', function(req,res){res.send("hallo")});
// Handle token grant requests
app.all('/oauth/token', app.oauth.grant());

//Handle accesstoken checks from the authorization server
app.get('/oauth/accesstoken', app.oauth.check());

//Handle logout. Delete accesstoken at server side.
app.get('/oauth/logout', app.oauth.logout());
