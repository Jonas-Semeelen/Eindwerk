 ...
 * @type {Array}
 */
 // Dit is de volgorde van methodes die wordt doorlopen als er een accestoken wordt aangevraagd.
var fns = [
  extractCredentials,
  checkClient,
...
function extractCredentials (done) {
  ...
  // credsFromBasic haalt het client_id + client_secret uit de header.
 ...
 * Dit stelt het device voor en een geheim dat bij het device hoort.
 ...
 */
 ...
 //auth(req) wordt uitgevoerd in /node_modules/basic-auth/index.js
 ...
 * @param  {Function} done
 * @this   OAuth
 * Modified so that every client is accepted.
 */
...
function saveAccessToken (done) {
  ...
  if (this.config.accessTokenLifetime !== null) {
    ...
    expires.setSeconds(expires.getSeconds() + this.config.accessTokenLifetime); // accessTokenLifetime == 3600 seconds
  }
...
function saveRefreshToken (done) {
  ...
  this.model.saveRefreshToken(refreshToken, this.client.clientId, expires, this.user, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
}
 ...
 * @this   OAuth
 */
function sendResponseToApp (done) {
    var response = {
      token_type: 'bearer',
