...
var pg = require('pg'),
  ...
  connString ='pg://postgres:root@localhost:5432/Authentication Server Database';
...
// Alle gebruikers worden aanvaard
...
model.saveAccessToken = function (accessToken, clientId, expires, userId, callback) {
  ...
  pg.connect(connString, function (err, client, done) {
    ...
    client.query('INSERT INTO oauth_access_tokens(user_id, client_id, access_token, expires) ' +
        'VALUES ($1, $2, $3, $4)', [userId, clientId, accessToken, expires],
        function (err, result) {
      if(err){
        client.query('UPDATE oauth_access_tokens SET access_token=$1, expires=$2, client_id=$3 WHERE user_id=$4', [accessToken, expires, clientId, userId],
 ...
 */
 // Aangepast zodat wachtwoord niet in plain tekst wordt opgeslagen
  ...
  pg.connect(connString, function (err, client, done) {
    ...
    client.query('SELECT salt FROM users WHERE username = $1', [username], function (err, result) {
      if(result.rowCount>0){
        ...
        unhashedString=username+':'+password+':'+salt;
        hashToCheck=crypto.createHash('sha256').update(unhashedString).digest('hex');
        console.log(hashToCheck);

        pg.connect(connString, function (err, client, done) {
          if (err) return callback(err);
          client.query('SELECT id FROM users WHERE username = $1 AND password = $2', [username,
              hashToCheck], function (err, result) {
...
model.checkAccessToken = function(accessToken, callback){
  var user;
  pg.connect(connString, function (err, client, done) {
    if (err) return callback(err);
    client.query('SELECT user_id FROM oauth_access_tokens WHERE access_token = $1', [accessToken], 
      function (err, result) {
        if(result.rowCount==0){
            user=undefined;
        }
...
//ZELF GESCHREVEN
model.deleteAccessToken = function(accessToken, callback){
  var resultaat;
  pg.connect(connString, function (err, client, done) {
    if (err) return callback(err);
    client.query('DELETE FROM oauth_access_tokens WHERE access_token = $1', [accessToken], 
      function (err, result) {       
        if(result.rowCount==0){
            resultaat=undefined;
        }
        else{
            resultaat='Accesstoken deleted';
        }
        callback(resultaat);
        done();
    });
